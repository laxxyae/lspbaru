...
@section('content')
    <h1 class="text-center text-dark mb-5">Tambah Sub Unit Kompetensi</h1>

    <p class="text-dark mt-3">Judul Unit Kompetensi: {{ $unitKompetensi->judul }}</p>

    <form action="/unit-kompetensi/{{ $unitKompetensi->kode_unit }}/sub" method="POST" class="text-dark">
        @csrf
        <div class="mb-3">
            ...
            <input type="text" class="form-control text-dark @error('judul') is-invalid @enderror" id="judul"
                name="judul" placeholder="Masukan Judul Sub Unit Kompetensi" value="{{ old('judul') }}" required
                autofocus>
            @error('judul')
