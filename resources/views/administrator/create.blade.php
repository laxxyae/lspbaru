...
@section('content')
    ...
    <form action="/administrator" method="POST" class="text-dark">
        @csrf
        <div class="mb-3">
            <label for="nama" class="form-label">Nama</label>
            <input type="text" class="form-control text-dark @error('nama') is-invalid @enderror" id="nama"
                name="nama" placeholder="Masukan Nama Disini" value="{{ old('nama') }}" required autofocus>
            @error('nama')
        ...
        <div class="mb-3">
            ...
            <input type="email" class="form-control text-dark @error('email') is-invalid @enderror" id="email"
                name="email" placeholder="Masukan Email Disini" value="{{ old('email') }}" required>
            @error('email')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
